...
async def run_ufw(*args) -> str:
    safe_args = validate_args(args)
    try:
        result = await asyncio.to_thread(
            run, ["ufw"] + safe_args, capture_output=True, text=True, check=True
        )
...
async def run_fail2ban(*args) -> str:
    safe_args = validate_args(args)
    try:
        result = await asyncio.to_thread(
            run, ["fail2ban-client"] + safe_args, capture_output=True, text=True, check=True
        )
        return result.stdout.strip()
    except CalledProcessError as e:
